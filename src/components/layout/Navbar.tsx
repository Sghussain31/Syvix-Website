...
export default function Navbar() {
  ...
  return (
    ...
    >
      <nav className="w-full max-w-7xl mx-auto flex h-20 items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="hidden md:inline-flex items-center justify-center rounded-lg border border-[#7C3AED]/60 bg-transparent px-4 py-2 text-xs font-bold uppercase tracking-wider text-slate-800 transition-all duration-150 hover:bg-[#7C3AED]/5 hover:border-[#7C3AED] hover:shadow-[0_4px_12px_rgba(124,58,237,0.1)] focus:outline-none focus:ring-2 focus:ring-[#7C3AED] active:scale-[0.98]"
          >
            Contact Us
          </button>
          <button
            ...
            className="flex h-10 w-10 items-center justify-center rounded-lg border border-slate-200/50 bg-white/40 text-slate-600 shadow-sm backdrop-blur-sm md:hidden hover:bg-white/60 focus:outline-none focus:ring-2 focus:ring-[#0066FF] transition-all duration-150"
      ...
      {mobileOpen && (
        <div className="border-t border-slate-200/50 bg-white/80 px-4 py-4 md:hidden backdrop-blur-md shadow-lg animate-slide-down">
          <ul className="flex flex-col gap-1.5">
            {NAV_WINDOWS.map((link) => (
              <li key={link.view}>
                <button
                  type="button"
                  onClick={() => navigate(link.view)}
                  className={`block w-full rounded-lg px-3 py-3 text-left text-xs font-bold tracking-wide uppercase transition-all duration-150 ${
                    activeWindow === link.view
                      ? "bg-[#0066FF]/10 text-[#0066FF]"
                      : "text-slate-600 hover:bg-slate-50/50 hover:text-[#0066FF]"
                  }`}
                >
                  {link.label}
                </button>
            ...
            <li className="mt-2 pt-2 border-t border-slate-200/50">
              <button
                type="button"
                onClick={() => {
                  openContactModal();
                ...
                className="w-full flex items-center justify-center rounded-lg bg-gradient-to-r from-[#0066FF] to-[#7C3AED] px-4 py-3 text-sm font-bold uppercase tracking-wider text-white shadow-md hover:shadow-lg active:scale-[0.98] transition-all duration-150"
              >
                Contact Us
              </button>
            </li>
